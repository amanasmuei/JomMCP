...
class ProductionReadinessTest:
    ...
    async def test_api_gateway_routing(self):
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                response = await client.get(f"{self.api_gateway}/api/v1/health/all", timeout=10)
                if response.status_code == 200:
                    data = response.json()
                    services = data.get('services', {})
                    print(f"   ✅ Service discovery working: {len(services)} services detected")
                    self.log_test("Service Discovery", "PASS", f"{len(services)} services")
                else:
                    print(f"   ❌ Service discovery failed: {response.status_code}")
                    self.log_test("Service Discovery", "FAIL", f"HTTP {response.status_code}")
            ...
            try:
                response = await client.get(f"{self.api_gateway}/api/v1/registrations", timeout=10)
                ...
                if response.status_code == 200:
                    ...
                    self.log_test("Redirect Handling", "PARTIAL", f"HTTP {response.status_code}")
            except Exception as e:
            ...
            try:
                ...
                response = await client.get(f"{self.api_gateway}/api/v1/generation", timeout=10)
                if response.status_code == 503:
                    print("   ✅ Proper 503 for unavailable service")
                    self.log_test("Error Handling", "PASS", "Proper 503 responses")
                else:
                    print(f"   ⚠️  Unexpected status: {response.status_code}")
                    self.log_test("Error Handling", "PARTIAL", f"HTTP {response.status_code}")
            except Exception as e:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if cors_origin == self.frontend_origin:
                        ...
                        self.log_test("CORS Configuration", "PASS", "Frontend origin allowed")
                    else:
                        print(f"   ⚠️  CORS origin: {cors_origin}")
                        self.log_test("CORS Configuration", "PARTIAL", f"Origin: {cors_origin}")
                else:
                    print(f"   ❌ CORS preflight failed: {response.status_code}")
                    self.log_test("CORS Configuration", "FAIL", f"HTTP {response.status_code}")
            except Exception as e:
                ...
                if login_response.status_code == 200:
                    ...
                    if list_response.status_code == 200:
                        ...
                        if create_response.status_code == 200:
                            ...
                            self.log_test("API Operations", "PASS", "CRUD operations working")
                        else:
                            print(f"   ⚠️  Create failed: {create_response.status_code}")
                            self.log_test("API Operations", "PARTIAL", "List works, create issues")
                    else:
                        print(f"   ❌ List registrations failed: {list_response.status_code}")
                        self.log_test("API Operations", "FAIL", f"HTTP {list_response.status_code}")
                        
                else:
                    print(f"   ❌ Authentication failed: {login_response.status_code}")
                    self.log_test("Authentication Flow", "FAIL", f"HTTP {login_response.status_code}")
            except Exception as e:
    ...
    async def test_api_endpoints(self):
        ...
        async with httpx.AsyncClient() as client:
            ...
            for endpoint, method, description in endpoints_to_test:
                ...
                try:
                    if method == "GET":
                        response = await client.get(f"{self.api_gateway}{endpoint}", timeout=10)
                    elif method == "POST":
                    ...
                    print(f"   Status: {response.status_code}")
                    if response.status_code in [200, 503]:  # 503 is expected for unavailable services
                        working_endpoints += 1
            ...
            self.log_test("API Endpoints", "PASS" if success_rate >= 80 else "PARTIAL", f"{success_rate:.1f}% success rate")
    ...
    def generate_report(self):
        ...
        for result in self.test_results:
            status_icon = {"PASS": "✅", "PARTIAL": "⚠️", "FAIL": "❌", "ERROR": "🔥"}[result['status']]
